...
class Dragon
  ...
  def ignore
    actions = ["eats your sofa.", "chases the mailman.", "gets stuck in the air vent.", "discovers a new passion for knitting.", "attempts to get the cookies from the top shelf on her own and falls.", "takes a bubble bath.","writes the next great American novel."]
  ...
  def yell_at_human
    ...
    case @color
    when "red"
      puts "ROARRRR!".colorize(:red)
    when "blue"
      puts "To be, or not to be?!".colorize(:blue)
    when "green"
      puts "purrr...".colorize(:green)
    when "yellow"
      puts "MEOWWW".colorize(:yellow)
    when "pink"
      puts "#{name} the magic dragon! Lives by the sea...".colorize(:pink)
    when "white"
      puts "WOOF".colorize(:white)
    when "black"
      puts "AM I DEATH AND DESTRUCTION?".colorize(:black).on_white
    end
  end
